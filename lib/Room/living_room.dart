...
class LivingRoomPage extends StatefulWidget {
  ...
  const LivingRoomPage({Key? key, required this.mqttClient}) : super(key: key);
  ...
  @override
  LivingRoomPageState createState() => LivingRoomPageState();
}

class LivingRoomPageState extends State<LivingRoomPage> {
  String uid = '';
  ...
  Future<void> _saveDeviceState(String device, bool state) async {
    ...
    await prefs.setBool('livingroom_$device', state);
  }

  Future<void> _loadDeviceStates() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      for (var device in LivingRoomPage.deviceNames) {
        deviceStates[device] = prefs.getBool('livingroom_$device') ?? false;
      }
    });
  }
  ...
  void _subscribeToDeviceTopics() {
    for (var device in LivingRoomPage.deviceNames) {
      final topic = 'users/$uid/room/Living/$device';
  ...
  void _handleMqttMessage(List<MqttReceivedMessage<MqttMessage>> event) {
    ...
    final Living = MqttPublishPayload.bytesToStringAsString(
      recMsg.payload.message,
    );
    final topic = event[0].topic;
    if (!topic.startsWith('users/$uid/room/Living/')) return; // ✅ Ignore other rooms
    final deviceName = topic.split('/').last;

    if (LivingRoomPage.deviceNames.contains(deviceName)) {
      final isOn = Living == '1';
      _saveDeviceState(deviceName, isOn); // 👈 Save state
      setState(() {
        deviceStates[deviceName] = isOn;
      });
      // print("Received update for $deviceName: $Living");
    }
  }

  void _publishDeviceState(String device, bool isOn) {
    final message = isOn ? '1' : '0';
    final topic = 'users/$uid/room/Living/$device'; // Example topic
