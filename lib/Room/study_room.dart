...
class StudyRoomPage extends StatefulWidget {
  ...
  const StudyRoomPage({Key? key, required this.mqttClient}) : super(key: key);
...
class _StudyRoomPageState extends State<StudyRoomPage> {
  ...
  Future<void> _saveDeviceState(String device, bool state) async {
    ...
    await prefs.setBool('studyroom_$device', state);
  }

  Future<void> _loadDeviceStates() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      for (var device in StudyRoomPage.deviceNames) {
        deviceStates[device] = prefs.getBool('studyroom_$device') ?? false;
      }
    });
  }
  ...
  void _subscribeToDeviceTopics() {
    for (var device in StudyRoomPage.deviceNames) {
      final topic = 'users/$uid/room/Study/$device';
  ...
  void _handleMqttMessage(List<MqttReceivedMessage<MqttMessage>> event) {
    ...
    final Study = MqttPublishPayload.bytesToStringAsString(
      recMsg.payload.message,
    );
    final topic = event[0].topic;
    if (!topic.startsWith('users/$uid/room/Study/')) return; // ✅ Ignore other rooms
    final deviceName = topic.split('/').last;

    if (StudyRoomPage.deviceNames.contains(deviceName)) {
      final isOn = Study == '1';
      _saveDeviceState(deviceName, isOn); // 👈 Save state
      setState(() {
        deviceStates[deviceName] = isOn;
      });
      // print("Received update for $deviceName: $Study");
    }
  }

  void _publishDeviceState(String device, bool isOn) {
    final message = isOn ? '1' : '0';
    final topic = 'users/$uid/room/Study/$device'; // Example topic
